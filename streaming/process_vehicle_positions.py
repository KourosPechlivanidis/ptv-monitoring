...
def get_write_vehicle_batch(config: VehicleConfig):
    ...
    def write_vehicle_batch(df: DataFrame, batch_id: int) -> None:
        """
        Maps and writes vehicle position fields to Redis.

        :param df: microbatch as a spark df that is written to redis
        :type df: DataFrame
        :param batch_id: identifier for the current micro-batch
        :type batch_id: int
        """
...
def main() -> None:
    ...
    df_enriched = join_gtfs_with_schedule(spark, df, config.route_paths, ["route_id"], ["route_id", "trip_id", "route_short_name", "route_long_name"])
